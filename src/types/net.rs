...
#[cfg(feature = "net_async")]
pub async fn async_receive<T: Convert, J: AsyncReadExt + Unpin + AsyncRead>(rx: &mut J) -> std::io::Result<T> {
    let mut buf = [0u8;5];
    let mut chunk = Vec::new();
    let mut packet = Vec::with_capacity(32);
...
#[cfg(feature = "net_async")]
pub async fn async_send<T: Convert, J: AsyncWriteExt + Unpin + AsyncWrite>(data: &mut T, tx: &mut J) -> std::io::Result<()> {
    let encoded = data.to_bytes_return();
    if encoded.len() > u32::MAX as usize {
        return Err(std::io::Error::from(ErrorKind::InvalidData))
    }
