...
impl<A, B, C, D> Convert for (A,B,C,D)
    where
        A: Convert, B: Convert, C: Convert, D: Convert
{
    fn to_bytes(&self, tx: &mut Vec<u8>) {
        self.3.to_bytes(tx);
        self.2.to_bytes(tx);
    ...
    fn from_bytes(rx: &mut Vec<u8>) -> io::Result<Self> {
        Ok((A::from_bytes(rx)?,B::from_bytes(rx)?,C::from_bytes(rx)?,D::from_bytes(rx)?))
    }
...
impl<A, B, C, D, E> Convert for (A,B,C,D,E)
    where
        A: Convert, B: Convert, C: Convert, D: Convert, E: Convert
{
    fn to_bytes(&self, tx: &mut Vec<u8>) {
        self.4.to_bytes(tx);
        self.3.to_bytes(tx);
    ...
    fn from_bytes(rx: &mut Vec<u8>) -> io::Result<Self> {
        Ok((A::from_bytes(rx)?,B::from_bytes(rx)?,C::from_bytes(rx)?,D::from_bytes(rx)?,E::from_bytes(rx)?))
    }
...
impl<A, B, C, D, E, F> Convert for (A,B,C,D,E,F)
    where
        A: Convert, B: Convert, C: Convert, D: Convert, E: Convert, F: Convert
{
    fn to_bytes(&self, tx: &mut Vec<u8>) {
        self.5.to_bytes(tx);
        self.4.to_bytes(tx);
    ...
    fn from_bytes(rx: &mut Vec<u8>) -> io::Result<Self> {
        Ok((A::from_bytes(rx)?,B::from_bytes(rx)?,C::from_bytes(rx)?,D::from_bytes(rx)?,E::from_bytes(rx)?,F::from_bytes(rx)?))
    }
