...
fn prepare_for_decode() -> Vec<u8> {
    simd_json::to_vec(&DATA.clone()).unwrap()
}
...
fn encode(b: &mut Bencher) {
    b.iter(|| {
        let _value = simd_json::to_vec(&DATA.clone()).unwrap();
    });;
}
...
fn decode(b: &mut Bencher) {
    b.iter(|| {
        let mut data = DATA_ENCODED.clone();
        let _decoded: Vec<Vec<SomeData>> = simd_json::from_slice(&mut data).unwrap();
    });;
}
